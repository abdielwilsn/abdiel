...
const Posts: React.FC = () => {
  ...
  const years = Array.from(new Set<string>(posts.map(p => {
          ...
          return (
            <section key={year}>
              <div className="relative h-10 flex items-center mb-4">
                ...
                <span className="text-xl font-medium opacity-20 ml-2">{year}</span>
              </div>
                ...
                {yearPosts.map(post => (
                  <li key={post.id} className="group">
                    <div className="flex flex-col md:flex-row md:items-baseline justify-between gap-1 md:gap-4">
                      <Link 
                        to={`/posts/${post.id}`} 
                        className="text-lg opacity-80 group-hover:opacity-100 group-hover:text-black dark:group-hover:text-white transition-all"
                      >
                        {post.title}
                      </Link>
                      <div className="flex items-center gap-3 text-xs opacity-30 group-hover:opacity-60 transition-opacity">
                        <span className="whitespace-nowrap">{post.date}</span>
                        <span className="w-1 h-1 bg-current rounded-full"></span>
                        <span className="whitespace-nowrap">{post.duration}</span>
                      </div>
                    </div>
                  </li>
                ))}
